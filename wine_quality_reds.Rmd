...
We can observe a very good correlation between fixed acidity and pH. This is
expected as pH is a measure of the total acidity in a solution.
...
We can observe a very good correlation between citric acid and pH. This is
expected as pH is a measure of the total acidity in a solution.
...
We can observe a very good correlation between total sulfur dioxide and free
sulfur dioxide. This is expected as the total.
...
The scatter plots allwed us to observe good correlation between the following
features:
- fixed acidity and citric acid
- volatile acidity and citric acid
- fixed acidity and pH
...
Some correlations are expected as some of the chemical properties are chemically
...
The strongest relationship for the quality of wine aws found with the volatile
acidity.
...
quality ~ volatile.acidity + alcohol + fixed.acidity +
          density + citric.acid + sulphates
...
- the absence of cross-validation: we have trained the model on one training set.
...
ggplot(data = wine, aes(x = quality, y = volatile.acidity, color = quality)) +
  geom_boxplot() +
  scale_color_brewer(palette = 'RdYlGn') +
  ylim(c(0.2, 1.2)) +
  labs(colour = 'Quality', x = 'Quality', y ='Volatile acidity') +
...
ggplot(data = wine, aes(x = citric.acid, y = volatile.acidity, color = quality)) +
  ...
  xlim(c(0, 0.8)) +
  ylim(c(0.2, 1.2)) +
  labs(colour = 'Quality', x = 'Citric acid', y ='Volatile acidity') +
  labs(title = 'Quality rating as a function of volatile acidity and citric acid')
```
...
and low volatile acidity (0.2 to 0.6 g/L). Low quality wines tend to be clustered
around low citric acid concentration (below 0.25 g/L) and high volatile acidity
(above 0.6).
...
ggplot(data = wine, aes(x = alcohol, y = volatile.acidity, color = quality)) +
  ...
  xlim(c(8, 14)) +
  ylim(c(0.2, 1.2)) +
  labs(colour = 'Quality', x = 'Alcohol', y ='Volatile acidity') +
...
and low volatile acidity (0.2 to 0.6 g/L). Low quality wines tend to be clustered
around low alcohol content (below 10 %) and high volatile acidity
(above 0.6).
...
to be good predictors. Naturally, one would think that those chemical properties,
associated with negative perception (taste or health), would be good indicators
of lower wine quality.
